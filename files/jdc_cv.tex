...
\usepackage{amsmath} % Required for multi-bib resource compatibility
...
%load bibliography resources (only from master bib)
\addbibresource{jdc_master.bib}
...
\newcounter{preprintcount}

\AtDataInput{%
  \ifkeyword{article}{\stepcounter{articlecount}}{}%
  \ifkeyword{review}{\stepcounter{reviewcount}}{}%
  \ifkeyword{book}{\stepcounter{bookcount}}{}%
  \ifkeyword{abstract}{\stepcounter{abstractcount}}{}%
  \ifkeyword{preprint}{\stepcounter{preprintcount}}{}%
}

\newcommand{\JDCReverseNumber}[2]{%
  \number\numexpr\value{#1}+1-#2\relax
}

\newcommand{\JDCSetNumbering}[1]{%
  \ifstrequal{#1}{article}
    {\DeclareFieldFormat{labelnumber}{\JDCReverseNumber{articlecount}{##1}}}
    {\ifstrequal{#1}{review}
      {\DeclareFieldFormat{labelnumber}{\JDCReverseNumber{reviewcount}{##1}}}
      {\ifstrequal{#1}{book}
        {\DeclareFieldFormat{labelnumber}{\JDCReverseNumber{bookcount}{##1}}}
        {\ifstrequal{#1}{abstract}
          {\DeclareFieldFormat{labelnumber}{\JDCReverseNumber{abstractcount}{##1}}}
          {\DeclareFieldFormat{labelnumber}{\JDCReverseNumber{preprintcount}{##1}}}
        }
      }
    }
}
...
\hyperref[secPUBLICATIONS2]{\hspace{0.4in} 2. Invited Reviews, Commentary, Chapters, Edited Volumes \& Technical Reports}
...
\label{secPUBLICATIONS1}

\begin{refsection}
    \nocite{*}

    \JDCSetNumbering{article}
...
% 2. Invited Reviews, Commentary, Chapters, Edited Volumes & Technical Reports

\subsection*{2. Invited Reviews, Commentary, Chapters, Edited Volumes \& Technical Reports}
\label{secPUBLICATIONS2}

\begin{refsection}
    \nocite{*}

    \JDCSetNumbering{review}
...
\subsection*{3. Books}
\label{secPUBLICATIONS3}

\begin{refsection}
    \nocite{*}

    \JDCSetNumbering{book}
...
\subsection*{4. Published Abstracts}
\label{secPUBLICATIONS4}

\begin{refsection}
    \nocite{*}

    \JDCSetNumbering{abstract}
...
% 5. Preprints: Manuscripts Under Review / In Preparation

\subsection*{5. Preprints: Manuscripts Under Review / In Preparation}
\label{secPUBLICATIONS5}

\begin{refsection}
    \nocite{*}

    \JDCSetNumbering{preprint}
...
AutoRA Workshop and Tutorial, Princeton University (2025)
...
Cognitive Science Society (1992, 2025)
...
Flatiron Analytic Connectionism Summer School (2025), Workshop on Computational Models of Human Intelligence
...
Stiftung Mercator, Mercator Fellow (2025-present)
