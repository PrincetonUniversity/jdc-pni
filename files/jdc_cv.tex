...
% ------------------------------------------------------------
% Count entries in each filtered bibliography and reverse the
% biblatex numbering so that the newest publication receives
% the highest number, matching the original CV.

\newrobustcmd*{\JDCReverseNumber}[1]{%
    \number\numexpr\csuse{JDCentrycount:\therefsection}+1-#1\relax
}

\DeclareFieldFormat{labelnumber}{\JDCReverseNumber{#1}}
...
% Load bibliography resources (CHANGED TO JUST THE MASTER BIBLIOGRAPHY
\addbibresource{jdc_master.bib}
...
\hyperref[secPUBLICATIONS]{PUBLICATIONS} %CHANGED
